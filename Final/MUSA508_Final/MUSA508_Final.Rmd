...
chicagoCensus <- 
  ...
  dplyr::select(Med_Inc, Percent_Vacancy, geometry) %>% 
  st_transform(ileast)
...
inspections_features$extracted_values <- extracted_values
...
vars_of_interest <- select_if(st_drop_geometry(inspections), is.numeric) %>% 
  na.omit()
...
model <- glm(fail_numeric ~ .,
                  data=modelTrain %>% 
                    dplyr::select(fail_numeric, Type, animals_nn5, sanitation_nn5, buildings_nn5, water_nn3, flooding_nn3, business_nn3, Med_Inc, Percent_Vacancy, extracted_values),
