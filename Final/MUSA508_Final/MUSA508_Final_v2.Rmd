...
inspections_2022 <- inspections_2022 %>%
    mutate(
      violent_crime_nn = nn_function(st_coordinates(inspections_2022), 
                                     st_coordinates(st_centroid(violent2021)), 
                                     k = 2),
      rats_nn = nn_function(st_coordinates(inspections_2022), 
                                     st_coordinates(st_centroid(rats2021)), 
                                     k = 5),
      sanitation_nn = nn_function(st_coordinates(inspections_2022), 
                                     st_coordinates(st_centroid(sanitation2021)), 
                                     k = 5)
      )
...
inspections_2023 <- inspections_2023 %>%
    mutate(
      violent_crime_nn = nn_function(st_coordinates(inspections_2023), 
                                     st_coordinates(st_centroid(violent2022)), 
                                     k = 2),
      rats_nn = nn_function(st_coordinates(inspections_2023), 
                                     st_coordinates(st_centroid(rats2022)), 
                                     k = 5),
      sanitation_nn = nn_function(st_coordinates(inspections_2023), 
                                     st_coordinates(st_centroid(sanitation2022)), 
                                     k = 5)
      )
...
inspections_22_23 <- rbind(inspections_2022, inspections_2023)
...
vars_of_interest <- select_if(st_drop_geometry(inspections_22_23), is.numeric) %>% 
  na.omit()
...
modelTrain <- inspections_2022_model
modelTest  <- inspections_2023_model
...
model1 <- glm(fail_numeric ~ .,
              data=modelTrain %>% 
                dplyr::select(fail_numeric, facility_type, Med_Rent, popdensity, winter, summer, Percent_Vacancy, neighborhood, quarter, rats_nn, sanitation_nn, violent_crime_nn, building_buffer, water_buffer, flooding_buffer, business_buffer, lagfail, prior_fails),
...
model2 <- glm(fail_numeric ~ .,
                  data=modelTrain %>% 
                    dplyr::select(fail_numeric, facility_type, neighborhood, quarter, Med_Rent, lagfail, prior_fails, popdensity, winter, summer),
                  family="binomial" (link="logit"))
...
results <- cbind(modelTest, testProbs)
...
```{r cv}
ctrl <- trainControl(method = "cv", number = 20, classProbs=TRUE, summaryFunction=twoClassSummary)

#Not running, "missing values in object"

inspections_22_23 <- inspections_22_23 %>%
  st_drop_geometry() %>%
  na.omit()


cvFit <- train(fail ~ .,
                  data=inspections_22_23 %>% 
                    dplyr::select(fail, facility_type, Med_Rent, Med_Inc, Percent_Vacancy, neighborhood, quarter, rats_nn, sanitation_nn, violent_crime_nn, building_buffer, water_buffer, flooding_buffer, business_buffer, lagfail, prior_fails, popdensity, winter, summer), 
...
ggplot()+
  ...
  geom_sf(data = streets_wrigleyville, color = "grey70", lwd = .5)+
  geom_sf(data = wrigleyville, fill = "lightblue", color = "slateblue1", lwd=1.5, alpha = .2)+
  geom_sf(data = wrigleyville_bounds_points, aes(color = Probs))+
