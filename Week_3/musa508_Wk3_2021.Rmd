...
```{r}
buffersAndTowns2 <- buffersAndTowns %>% 
  filter(Legend == "Inside")

# Calculate label positions (this is a simple example; you may need to adjust these)
buffersAndTowns2$label_x <- st_coordinates(st_centroid(buffersAndTowns2$geometry))[, 1]
buffersAndTowns2$label_y <- st_coordinates(st_centroid(buffersAndTowns2$geometry))[, 2]

ggplot() + 
  geom_sf(data = buffersAndTowns, aes(fill = MUNI, alpha = Legend)) +
  scale_alpha_manual(values = c("Inside" = 0.5, "Outside" = 1)) +
  geom_label_repel(data = buffersAndTowns2, aes(x = buffersAndTowns2$label_x, y = buffersAndTowns2$label_y, label = MUNI), 
                      box.padding = 0.1,
                      size = 1, color = "black") +
  labs(title = "Buffers and Towns") +
  mapTheme()
```
