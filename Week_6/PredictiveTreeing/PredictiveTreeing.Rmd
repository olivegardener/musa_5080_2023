...
output: html_document:
    code_folding: hide
---
Build a version of the spatial risk model seen in the book (and in the Week 6 "Predictive Policing" lab) for a different outcome that likely suffers from more selection bias than burglary.
...
### A map of your outcome of interest
...
ggplot() +
  ...
  mapTheme()
```
Joining the count of street tree 311 requests to the fishnet reveals some hotspots, where single cells are bright with as many as 20 requests! Could this be indicative of actual concentrations of tree work needing to be done, or perhaps just a small handful of persistent citizens?
...
It involves partitioning the original dataset into a training set to train the model and a test set to evaluate it.
...
```
These two maps indicate that our predicted calls are more spread out across the city than the actual density for 2019.
...
I would not recommend this algorithm be used to predict where Street Tree 311 calls will be placed. The algorithm, when trained on 2018 data, failed to predict calls for 2019 data with acceptable accuracy. Though the average percent error was small, the map and bar chart above indicate that the algorithm over-estimates risk in the lower risk areas and underestimates risk in higher risk areas. This is the opposite of what one would



Using such predictions to inspect trees prior to issues arising with them might benefit the city. This amount of 'false positive' predictions, if used to send crews out preventatively, could lead to reductions in property damage and cost reductions to the city since fewer crews would be sent out on emergency removal jobs.

After all, as the famous Ben Franklin quote goes, 'an ounce of prevention is worth a pound of cure.'
