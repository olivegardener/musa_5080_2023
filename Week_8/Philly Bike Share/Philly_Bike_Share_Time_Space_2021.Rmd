...
Bikeshare is... here's why supply and demand is challenging and why it matters for bikeshare.

the best way to balance bikes is to ride the rush hour peaks in reverse, bringing bikes from docks at fringe stations to core stations, since core stations have more churn.
Gamify it through incentivizing riders to bring bikes back to their origin stations at the end of the day. 
and incentivize users to move bikes from full stations to empty stations during off-peak hours to reduce need of trucks bringing bikes to and fro.

Predicting two weeks into the future would balance the competing needs of accuracy and proactive scheduling of employees.
...
```
Here is the weather data, showing how temp and wind speet are somewhat tied together and fluctuate with the day-night swing.
...
ggplot(dat_census %>%
  ...
  labs(title="Bike share trips per hr. Philadelphia, May-June, 2023, 2018",
       x="Date", 
       y="Number of trips")+
  plotTheme
```
This plot shows a relatively consistent daily oscillation, with two peaks on each day of the work week - one in the morning and a larger one in the evening rush hour.
...
dat_census %>%
  ...
  labs(title="Mean Number of Hourly Trips Per Station. Philadelphia, May, 2018",
       x="Number of trips", 
       y="Frequency")+
...
ggplot(dat_census %>%
  ...
  labs(title="Bike share trips per hr by station. Philadelphia, May, 2018",
       x="Trip Counts", 
...
ggplot(dat_census %>% 
  ...
  labs(title="Bike share trips in Philadelphia - weekend vs weekday, May, 2018",
     ...
     plotTheme
```
one peak on weekends versus two on weekdays. lower ridership on average on weekends. more need for balancing on weekdays. sunday evening is the most important time to rebalance
...
This focus on trips by stations reflects features not otherwise captured in the model, such as amenities near each station. Given that amenities are fixed in both time and space, they are captured by station locations themselves rather than needing to be modeled explicitly. Inclusion of fixed amenities would result in mutli-colinearity, since the amenities do not vary across time.
...
ggplot(.)+
  ...
  mapTheme
```
This plot shows higher errors at the core of the network than the fringe.

If our MAE is 0.65, this means...
...
```
Tighter histogram would be more generalizable because it means our model is not producing significantly different results than those of the random samples.
...
Our model is not good enough because it is not great a capturing peak times. Comparing mae of the trip count to the mean of the trip count shows...

Our data looks at May to June. Given the high number of higher education institutions in Philly, it is important to consider how the dip in the city's population will be reflected in ridership, particularly in areas of the city with higher proportions of students and univeristy buildings.
