...
ggplot(dat_census %>% 
  ...
  labs(title="Bike share trips in Philadelphia - weekend vs weekday, May, 2018",
     ...
     plotTheme
```
Richard's code from ed discussion
```{r}
# 
# all_data_day <-
#   dat_census %>%
#   group_by(dotw,start_station,geometry) %>% summarise(sum_day_time = sum(sum_rides))
# 
# animate_map = ggplot()+
#   geom_sf(data=all_data_day,aes(color=sum_day_time),size=1.2)+
#   scale_color_viridis(name='Number of Rides')+
#   transition_states(day)+
#   ggtitle('Day: {frame_time}')
#   
# animate(animate_map,nframes=41)
...
reg.spatialCV <- crossValidate(
  dataset = fiveweekpanel,
  id = "start_station",
  dependentVariable = "dotw",
