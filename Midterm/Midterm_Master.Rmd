...
```
There are a lot of variables in this model that appear to have low significance. 
...
```
From this correlogram, we can see that there are some potentially co-linear variables and variables with low correlation with sale_price. Let's remove these variables.

```{r}
Houses <- Houses %>% dplyr::select(-number_stories, -age, -PopDensity_ACS, -Car2WorkPct_ACS, -dist2trail, -dist2park, -hospitals_nn1, -rr_station_nn1, -septa_trolley_nn1, -septa_highspeed_nn1, -unis_nn1, -landcare_lots_nn5)

```

```{r regression training set}
reg2 <- lm(sale_price ~ ., data = select_if(st_drop_geometry(Houses), is.numeric))
summary(reg2)
