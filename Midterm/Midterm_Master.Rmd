...
# Example: Creating three equal-width bins
Philly_Data.test$sale_price.Error.Bin <- cut(Philly_Data.test$sale_price.Error, 
                                             breaks = 3, 
                                             labels = c("Low", "Medium", "High"))

ggplot() +
  geom_sf(data = neighborhoods, fill = "white") +
  geom_sf(data = Philly_Data.test, 
          aes(fill = sale_price.Error.Bin),  # Use the binned variable
          size = .01, alpha = .8) +
  scale_fill_manual(values = c("Low" = "red", "Medium" = "yellow", "High" = "blue"), 
                    name = "Residuals") +
  labs(title = "Binned Residuals of Sale Price Error") +
  mapTheme()
