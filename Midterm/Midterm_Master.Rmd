...
```
### Dave to look into organizing stargazer table by internal characteristics, amenities/public services or spatial structure
...
ggcorrplot(
  ...
  theme(
    title = element_text(size = 6),            # Title text size
    axis.title = element_text(size = 6),        # Axis title text size
...
ggplot() + geom_sf(data = nhoods, fill = "white") +
  stat_density2d(data = data.frame(st_coordinates(Houses_age)), 
                 aes(X, Y, fill = ..level.., alpha = ..level..),
                 size = 0.01, bins = 40, geom = 'polygon') +
  scale_fill_gradient(low = "#E1B93C", high = "#a02300", 
...
lapply(Houses, function(x) if(is.factor(x)) levels(x))
