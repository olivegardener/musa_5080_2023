...
playgrounds <- st_read("data/PPR_Playgrounds.geojson") %>% 
  st_transform(crs = coordinate_system)
...
regional_rail <- st_read("data/Regional_Rail_Stations.geojson") %>% 
  st_transform(crs = coordinate_system) 
regional_rail <- st_intersection(philly_outline, regional_rail)

rail_stops <- st_join(light_rail, regional_rail)

universities <- st_read("data/uni.geojson") %>% 
  st_transform(crs = coordinate_system)
hospitals <- st_read("data/Hospitals.geojson") %>% 
  st_transform(crs = coordinate_system)

eds_and_meds <- st_join(universities, hospitals)
...
model_data <- model_data %>% 
          ...
          st_join(dplyr::select(food_access, TOTAL_HPSS, TOTAL_RESTAURANTS)) %>% #HPSS is high produce supply store
          st_join(dplyr::select(tracts2020, MedHHInc, MedRent, PctWhite, PctBach)) %>%
          rename(
...
model_data <- model_data %>%
    mutate(
      rail_stop_nn1 = nn_function(st_coordinates(model_data), 
                                     ...
                                     k = 1),
      vacant_lots_nn5 = nn_function(st_coordinates(model_data), 
                                    st_coordinates(st_centroid(vacant_lots)), 
                                    k = 5),
      playgrounds_nn2 = nn_function(st_coordinates(model_data), 
                                    st_coordinates(st_centroid(playgrounds)), 
                                    k = 2),
      )
...
# CanopyPct100ft <- vector("numeric", length = nrow(model_data))
# 
...
model_data <- model_data %>%
  mutate(
    ...
    dining = ifelse(is.na(dining), median(dining, na.rm = TRUE), dining),
    MedHHInc_ACS = ifelse(is.na(MedHHInc_ACS), median(MedHHInc_ACS, na.rm = TRUE), MedHHInc_ACS),
    MedRent_ACS = ifelse(is.na(MedRent_ACS), median(MedRent_ACS, na.rm = TRUE), MedRent_ACS),
    ...
    CanopyPctQuMi = ifelse(is.na(CanopyPctQuMi), median(CanopyPctQuMi, na.rm = TRUE), CanopyPctQuMi),
  ...
  filter(sale_price < 1500000) # do we like this as filter

glimpse(model_data)
```
!!!!!!!!!!convert some dbl factors to int?
...
st_drop_geometry(model_data) %>% 
  select(sale_price, vacant_lots_nn5, playgrounds_nn2, quality_produce_access, tree.count.buffer) %>%
...
#   geom_sf(data = model_data, aes(colour = q5(sale_price)), 
#           show.legend = "point", size = .01) +
#   scale_colour_manual(values = palette_sequential,
#                    labels=qBr(model_data,"sale_price"),
#                    name="Quintile\nBreaks") +
...
```{r map3, fig.width=6, fig.height=4}

playgrounds <- playgrounds %>%
  select(OBJECTID, geometry)

neighborhoods$playground_count <- neighborhoods %>% 
    aggregate(mutate(playgrounds, counter = 1),., sum) %>%
    pull(counter)

ggplot() + geom_sf(data = neighborhoods, aes(fill = playground_count))+
  scale_fill_gradient(low = "lightblue", high = "darkblue", name = "Count") +
  labs(title = "Playgrounds per Neighborhood, Philly",
       subtitle = "Philly Parks and Rec Sites",
       legend = "Count") +
  mapTheme()
...
ggplot()+
  geom_sf(data=neighborhoods, fill = "white")+
  geom_sf(data=Philly_Data.test, aes(color = sale_price.Error), size = .01, alpha = .8)+
  scale_color_gradient(low = "red", high = "yellow", name = "Residuals") + 
  mapTheme()
...
#   dplyr::select(-toPredict, -dining,)
# 
# Philly_Data.predict <- rbind(Philly_Data, Philly_Data.challenge)
# 
# Philly_Data.challenge <-
#   Philly_Data.challenge %>%
#   mutate(Regression = "Regression",
#          sale_price.Predict = predict(reg.training, Philly_Data.predict),
#          sale_price.Error = sale_price.Predict - sale_price,
#          sale_price.AbsError = abs(sale_price.Predict - sale_price),
#          sale_price.APE = abs((sale_price.Predict - sale_price) / sale_price.Predict))
# 
...
#   geom_sf(data = Philly_Data.test, aes(colour = q5(sale_price.Predict)), 
#           show.legend = "point", size = .01) +
#   scale_colour_manual(values = palette_sequential,
...
```
This shows that the model is good at predicting across much of the city, with errors clustered in a small handful of errors. Why is the absolute percent error so high in these neighborhoods?
...
scatterplot <- ggplot(neighborhood_summary, aes(x = mean_price, y = mean.MAPE)) +
  geom_point(aes(color = mean.MAPE), size = 3) +
...
st_join(Philly_Data.test, tracts21) %>% 
