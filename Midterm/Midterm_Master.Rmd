...
To support Zillow’s goal of more accurate housing market predictions, our goal with this analysis is to predict single family housing prices in Philadelphia, PA through the use of local data sources. This task is challenging due to the multifaceted nature of the housing market. House prices are influenced by both internal and external characteristics and are subject to the unruly forces of market dynamics, which add a layer of turbulence to the data. Not everyone is a rational actor...

However, accurately estimating home values is hugely important because the cost of housing plays a critical role in the lives of residents. The value of a home is important to its owner because if overestimated, it may place undue burden on the homeowner through higher property taxes. On the other hand, underestimation of housing values leads to the devaluation of one of the largest investments in most people’s lives. As a result, it is critical to estimate the fair market value of houses by leveraging a wide variety of variables that influence home value.

So why is this a difficult exercise? It is impossible to account for every factor in a model of housing price. Additionally, there is the risk of involving too many factors in the model, which results in estimates that are exceptionally accurate for the available data, but bad for data outside of the model. This is known as “overfitting” a model, which results in poor generalizability.

The overall modeling strategy employed in this analysis involves the collection of a range of internal and external characteristics of individual houses and then using statistical methods to estimate the value of individual Philadelphia homes in the dataset. 

The starting point for this model was assessment data from the City of Philadelphia, which included sale prices and a variety of other internal characteristics for every house in Philadelphia. We also included a range of external data with characteristics of the broader urban landscape. Using a number of statistical methods, we summarized these characteristics in relation to each individual house in the assessment data. Once these internal and external characteristics were compiled into a single dataset, we explored relationships between the various factors and sale price, narrowing them down the ones which were the most impactful for home values.

We then split the data into two groups, a “training set” and a “test set.” The training set was used to develop a model for predicting sale price for homes in Philadelphia based on the range of factors we felt most influenced home price. The resulting model was then used to predict sale prices of houses in the test set, and the actual and predicted sale prices were compared.

When we used our model to predict sale prices across Philadelphia, it performed relatively well for areas with predominantly higher income, white populations. However, it struggled in areas with majority non-white and/or lower income populations.

# Data

Data used in this model were gathered from a variety of sources and joined into a single dataset for use in building a linear regression model for predicting home sale price. To supplement the original dataset provided for this project - housing assessment data for Philadelphia - we used a mix of API calls and direct downloads to gather the data.
...
The remainder of the data was downloaded from Open Data Philly previously for use in a separate project and therefore was imported as a geodatabase. These datasets included information for vacant lots, recycling diversion rates, hospitals, regional rail stations, trail networks, trolley stops, and traffic collisions. Data was then merged as needed, such as combining trolley stops, subway stops and regional rail stations together to form a “rail station” dataset.

A cleaned version of the housing assessment data for Philadelphia was created, containing only the variables we felt would be most influential on single family home prices:
...
Information from a given area, such as data from Census tracts, was added to individual points to contextualize the areas around a given home,
The distance was calculated from each house to its nearest park,
Tree totals were incorporated by counting all trees within 500 feet of each house,
Rail stops and vacant lots were incorporated through a ‘K nearest neighbors’ function, which averaged the distance to the nearest 1 and 3 points, respectively. 

All incomplete variable fields were then filled as the median value from the full dataset for each variable. The joined data was then narrowed down by eliminating variables that were deemed too similar to each other through statistical methods, as well as variables that had a weak relationship with the sale price of homes.
...
sumtable(model_data_table, out = "kable") %>%
  scroll_box(width = "100%", height = "450px",
             fixed_thead = TRUE) %>% kable_styling(full_width = TRUE)
```
...
This correlation plot shows the relationship between variables in the model - values closer 1 or -1 mean that those two variables are similar in their predictive power on sale price.
...
After compiling our data and developing features that we believed were predictors for sale price, we conducted a series of statistical analyses to test the accuracy of our prediction. The type of model used was a linear regression, which allows us to examine the individual impact of each predictor on the sale price of a home in the Philly data set when holding all other predictors equal. 

We split our data into two random groups for a training set and a test set - doing so allowed us to create a sale price prediction model using the training data to predict on the test data. This split let us use our model on data with known house sale prices, but with combinations of predictor variables that the model has not necessarily encountered. We then compared the sale price predictions in the test data to the actual sale prices and analyzed the difference between our predictions and reality.

Additional statistical methods were then used to compare our results and check if our testing data is representative of the city as a whole:
- We calculated the absolute error and the absolute percent error (APE), which are key figures that show us how much our predictions differ from actual sales prices - the means of each figure were also calculated to show us how well our model performed across different areas of the city. 
 - We checked if the random group assigned to the test data would give us similar results to other randomly assigned test sets through cross validation - this let us confirm that our test data didn’t wildly differ from the rest of the city.
 - We compared how much the price errors for an individual house different to neighboring houses’ sale price errors and examined if our sale price errors show clustering, which would highlight the need for spatial solutions to accurately map home prices in the city.

Through the mean absolute percent error (MAPE), we saw which neighborhoods our model predicted accurately, with a lower score meaning that the model is more accurate. We also used MAPE to examine the generalizability of our model on different demographics to see how our model performed in different contexts - in this case low income versus high income neighborhoods and majority white versus majority non-white neighborhoods.
...
# smaller table for rsquared only in case where condensed results table below is used

# rsquare <- summary(reg.training)$r.squared
# rsquareadj <- summary(reg.training)$adj.r.squared
# 
# corrval <- cbind(rsquare, rsquareadj)
# 
# kbl(corrval, col.names = c('R^2', 'R^2 Adjusted'), digits = 3) %>% kable_styling(full_width = FALSE)
```
...
# Condensing with kable table means the footnote with rsquared disappears at end
...
This table shows our regression model results, including how well our model fits the data and the significance and impact of each individual variable on sales price when all other predictors are unchanged: 

- The adjusted R² value is used due to the high number of variables caused by all neighborhoods being included in the regression model. This value tells us that 76% of the variability in sales price is explained by our model. On face value, this relatively high percentage means our model fits the data well, but adjusted R² alone doesn't tell us if this model is accurate - as shown later our chosen variables may be biased towards certain demographic and income contexts.

- The "Est." column shows us how much each variable impacts sales price when all other variables are held equal, with the "p" column indicating to us whether that variable is considered statistically significant within the context of the model most of our chosen variables and many of the individual neighborhoods are significant in their impacts on ale price, with p values of less than 0.05. Variables are measured in different ways - factors like the condition are representative of codes, where a 1 represents the best condition and each additional number means the house is in worse condition. Other variables like total livable area show the impact of on on additional square foot on the sale price - the estimate indicated that price increases by \$132 for every added square foot.
...
The process of cross-validating the results of our test set by generating the mean average errors of 100 groups of training data allows us to confirm if the MAE and MAPE from our test data is expected from our model or outside of the norm as a result of taking a random sample of the data for training. 

- The table above shows that our test data MAE of nearly \$60,000 is nearly equal to the average generated by our cross validation test, which has a standard deviation of about \$5,000.
...
Plotting the 100 cross validation mean averages errors shows the data centered around the overall mean of about \$60,000 and highlights how test data MAEs are likely to range between about \$50,000 and \$70,000 - about two standard deviations in either direction from the mean.  
...
Plotting the predicted sale price as a function of actual sale prices and adding lines that best fit the data show us that our predictions should be very close to accurate. However, the previous figures for MAPE told us that our predictions differed from the actual sale price by about 40% on average. As highlighted in later figures, we likely predict many neighborhoods relatively well and some neighborhoods very poorly.
...
ggplot() +
  ...
  geom_sf(data = Philly_Data.test, aes(colour = q5(sale_price.Error)), 
          show.legend = "point", size = .1, alpha = .5) +
...
Conducting the Moran's I test lets us see how much the data in our test set is clustered - in other words, is the difference between the actual and predicted sale price of a particular home similar to other homes neighboring it? This test creates simulations of the test data by shuffling the results of differences to be randomly distributed across the city, plotting the results of the test as a histogram and the comparing it the actual observation of Moran's I in out test data. Our test result of Moran's I tells us that differences do cluster based on geography.
...
The spatial lag in errors plot compares a house's sale price error to its neighbors - the line shows that the relationship is positive, further reinforcing that there is clustering of errors in our test data.
...
Mapping the predicted sale prices side by side with the actual sale prices can let us compare what our model predicts home prices to be versus reality. The maps appear to be subtly different, with a few more areas that look like darker or lighter on the prediction map - this reinforces how our model lightly over-predicts sale price in certain areas and severely under-predicts in others. Given the sheer volume of observations in our data set, it is difficult to highlight specific points to compare in a city-wide map.  
...
This plot reinforces our observations from the MAPE map. For most neighborhoods, the average difference between predicted and observed sale price is relatively low, but one neighborhood in particular is a large outlier and about 4-5 times higher than the next highest mean MAPE for a neighborhood. 
...
Adding our test data to the Philly Census tracts allows us to compare the prediction results from our model across the contexts of race and income in the city. These tables show that our model performs with much lower accuracy in neighborhoods that have higher proportions of residents of color and lower income residents.

# Discussion

Ultimately, this model is not effective at predicting home prices. Predictions were off by more than half the value of the home across the city as a whole, and in some cases the difference was as much as the total actual value of a home.

Most of the variables used in the model were statistically significant, but certain variables had interesting effects on the home price predictions. Internal characteristics of a home, such as internal or external condition, had some of the biggest impacts in terms of dollars, but it was interesting to see that the HOLC grades, which represent the historical redlining in Philadelphia, dropped the price by over \$6,000 when moving grade A (the historical highest) down a level through the lowest grade. Quality produce and low produce access were also interesting to see as impactful variables - each represents the count of stores nearby that either have high quality produce selections or minimal produce selections and had a more meaningful impact than expected. Finally, it was surprising to see the impact of one additional tree within a 500 foot radius - a nearly \$330 increase per tree.

Based on the R-squared value of this model, it was able to predict  about 76% variation in prices. This relatively high led us to initially believe that our model fit the data well, but we ultimately struggled to account for prices in certain areas. The high R-squared value, but poor generalizability highlights the pitfalls in chasing after a high percentage, because in the end, it doesn’t necessarily reflect the predictive quality of the model.
The variables with the largest impact on price were internal characteristics, especially the interior condition of a home, which could drop the price by nearly \$29,000 per condition value, which as mentioned indicated a worse condition with a higher value. In terms of dollars for a coefficient, fireplaces were surprisingly impactful, with the presence of one fireplace influencing the price of a home by an increase of over \$23,000. 

Predicted home price relative to actual home price resulted in a MAE and MAPE of…

We had a relatively consistent amount of absolute error (under- or over-estimation of sale price) across much of the city. However, our model was consistently highly inaccurate in one or a small handful of neighborhoods, which were often lower income, non-white majority neighborhoods. 

While our comparison scatterplot of actual versus predicted sale price appears to show a strong fit, the MAPE map shows how we could often not account for the spatial variation in particular neighborhoods. This could be due to certain neighborhoods having lower numbers of observations to train on. It could also be due to variation in price in this neighborhood not relating to the variables of our model in the same way that occurs in other neighborhoods. Each time we ran the model, the neighborhood with the inaccurate predictions

It's possible that in splitting the data between testing and training datasets, some neighborhoods (particularly ones with a lower number of homes) might end up being under-represented in the training data. We also chose to impute values for missing data as the median value for each variable across the whole dataset, which may not affect the results for the city as a whole but may impact the predictions for individual neighborhoods or areas of the city with larger proportions of missing data. Assessment data might be spatially incomplete, with better data available in wealthier, whiter neighborhoods, which given our methods for imputing missing data would result in skewed imputed values.
...
This model could be improved through use of automated feature selection, neural nets, and other contemporary approaches to prediction. Short of these changes, the model would benefit from us spending more time refining which independent variables are used in the model and perhaps incorporating more variables that account for fluctuations in price across lower income neighborhoods and predominantly non-white neighborhoods and variables that account for ‘market hotness.’

Additionally, the methods used to impute missing values in the data could be improved by better accounting for spatial variation. For instance, rather than using the median of the entire dataset, imputing the value using the median of the 10 or so nearest neighbors or the neighborhood as a whole.
