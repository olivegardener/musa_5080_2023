...
# Change crs to include
trees2 <- st_read("data/trees_OA.geojson") %>%
  na.omit() %>%
  st_as_sf(coords = c("LOC_X", "LOC_Y"), crs = "EPSG:4326") %>%
  st_transform(crs = coordinate_system) %>%
  st_intersection(philly_outline, trees2) %>%
  dplyr::select(-TREE_NAME, -LOC_Y, -LOC_X)
...
#   stat_density2d(data = data.frame(st_coordinates(trees2)), 
#                  aes(X, Y, fill = ..level.., alpha = ..level.., weight = trees2$TREE_DBH),
