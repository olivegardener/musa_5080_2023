...
Houses_Clean %>%
  mutate(PctBach_ACS = PctBach_ACS*100,
         Houses_Clean$exterior_condition[is.na(Houses_Clean$exterior_condition)]<- median(Houses_Clean$exterior_condition,na.rm = TRUE),
...
inTrain <- createDataPartition(
              y = paste(Philly_Data$holc_grade),
              p = .60, list = FALSE)
...
Philly_Data.test <-
  ...
  na.omit() #should NA omit be here?
...
```{r mape map}
Philly_Data.test %>%
  group_by(Regression, holc_grade) %>%
  ...
  st_join(neighborhoods) %>%
    ...
    ggplot() + 
      ...
      geom_sf(data = regression_data, colour = "black", size = .5) +
