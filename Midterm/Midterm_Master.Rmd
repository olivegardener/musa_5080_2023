...
model_data$trees.buffer <- model_data %>% 
    st_buffer(500) %>% 
...
model_data <- model_data %>%
  mutate(
    ...
    trees.buffer = ifelse(is.na(trees.buffer), median(trees.buffer, na.rm = TRUE), trees.buffer),
...
```{r scatterplots, fig.width=8, fig.height=8}

st_drop_geometry(model_data) %>% 
  select(sale_price, vacant_lots_nn5, playgrounds_nn2, quality_produce_access, trees.buffer) %>%
