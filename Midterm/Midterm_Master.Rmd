...
palette_three <- c("#f1a340", "#f7f7f7", "#998ec3")
...
## Correlation matrix
```{r correlation matrix, fig.width=10}
...
This correlation plot shows the relationship between variables in the model - values closer 1 or -1 mean that those two variables are similar in their predictive power on sale price - we used correlation plots to trim variables that had collinearity of .8 or more.
...
Map of single family home sale prices as reflected by the provided assessment data.
...
Density map of the locations of trees - the most densely wooded areas are generally closer to Center City. One limitation with this data set is that certain large parks in the city do not have their trees counted by Philadelphia Parks and Recreation, who provides the data for this data set.
...
High quality produce access shows which areas of the city have more stores that are considered to have a high supply of high quality and fresh produce, which is used to determine healthy food access. These stores also appear to cluster around Center City.
...
This heat map shows areas with high numbers of vacant lots based on their locations - there are high concentrations of vacant lots in northern and western parts of the city.   
...
- The table above shows that our test data MAE of nearly \$60,000 is nearly equal to the average generated by our cross validation test, which has a standard deviation of about \$4,500.
