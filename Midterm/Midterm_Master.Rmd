...
Houses_Clean %>%
  mutate(PctBach_ACS = PctBach_ACS*100,
         exterior_condition2 = replace(Houses_Clean$exterior_condition, is.na(Houses_Clean$exterior_condition), 100000),
         Houses_Clean$fireplaces[is.na(Houses_Clean$fireplaces)]<- median(Houses_Clean$fireplaces,na.rm = TRUE),
         Houses_Clean$interior_condition[is.na(Houses_Clean$interior_condition)]<- median(Houses_Clean$interior_condition,na.rm = TRUE),
         Houses_Clean$number_of_bathrooms[is.na(Houses_Clean$number_of_bathrooms)]<- median(Houses_Clean$number_of_bathrooms,na.rm = TRUE),
         Houses_Clean$number_of_bedrooms[is.na(Houses_Clean$number_of_bedrooms)]<- median(Houses_Clean$number_of_bedrooms,na.rm = TRUE),
         Houses_Clean$total_area[is.na(Houses_Clean$total_area)]<- median(Houses_Clean$total_area,na.rm = TRUE),
         Houses_Clean$total_livable_area[is.na(Houses_Clean$total_livable_area)]<- median(Houses_Clean$total_livable_area,na.rm = TRUE),
         Houses_Clean$PctBach_ACS[is.na(Houses_Clean$PctBach_ACS)]<- median(Houses_Clean$PctBach_ACS,na.rm = TRUE),
         Houses_Clean$septa_bus_nn5[is.na(Houses_Clean$septa_bus_nn5)]<- median(Houses_Clean$septa_bus_nn5,na.rm = TRUE),
         Houses_Clean$holc_grade[is.na(Houses_Clean$holc_grade)]<- "no grade",
          Houses_Clean$dist[is.na(Houses_Clean$dist)]<- "no district")
...
Philly_Data.test %>%
  ...
  st_join(neighborhoods) %>%
    ...
    ggplot() + 
      geom_sf(data = neighborhoods, aes(fill = mean.MAPE)) +
      geom_sf(data = regression_data, colour = "black", size = .1) +
...
ggplot() + 
  geom_sf(data = neighborhoods, aes(fill = mean.MAPE)) +
  geom_point(data = Philly_Data.test, aes(x = lon, y = lat, color = black, size = .1)) +
  scale_color_gradient(low = "blue", high = "red", name = "MAPE") +
  labs(title = "Mean test set MAPE by neighborhood") +
  theme_minimal()
