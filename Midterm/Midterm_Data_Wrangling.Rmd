...
phl <- counties(state = 42) %>% 
  filter(NAME == "Philadelphia") %>%
  st_transform(counties, crs = 2272)
...
bikes <- get_spatial_layer("https://services.arcgis.com/fLeGjb7u4uXqeF9q/arcgis/rest/services/Bike_Network/FeatureServer/0/") %>% 
  st_transform(crs = 2272)
...
#struggling to load on my comp
tree_canopy_change_08to18 <- get_spatial_layer("https://services.arcgis.com/fLeGjb7u4uXqeF9q/arcgis/rest/services/TreeCanopyChange_2008_2018/FeatureServer/0/")%>%
  st_transform(crs = 2272)
...
prop <- get_spatial_layer("https://phl.carto.com/api/v2/sql?q=SELECT * FROM opa_properties_public")
...
Building_Pts <- Buildings$osm_points
```
...
# Create the first plot
Buildings_Poly <- ggplot() +
  geom_sf(data = Dist2Highway)+
          # aes(fill = Dist2Highway),  # Fill color based on Dist2Bart
          # color = NA,
          # alpha = 0.75) +
  geom_sf(data = highways, color = "black") +
