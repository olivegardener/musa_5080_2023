...
# Compute the minimum distance for each point to the nearest polygon
min_distances <- apply(distances, 1, min)
# Add the minimum distances to the attribute table of the points
...
Houses <- Houses %>%
    mutate(hospitals_nn1 = nn_function(st_coordinates(Houses), 
           ...
           # trees_nn20 = nn_function(st_coordinates(Houses), 
           #                    st_coordinates(trees), k = 20),
...
vars_of_interest <- select_if(st_drop_geometry(Houses), is.numeric) %>% 
  select(-musaID, -sale_YYYY.Q, -dist2highway, -HomeOwnRate_ACS) %>% 
  na.omit()
