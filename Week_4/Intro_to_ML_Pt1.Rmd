...
knitr::opts_chunk$set(echo = TRUE, retina = 300)
...
boston.sf %>%
  ...
  gather(Variable, Value, -SalePrice) %>% 
   ggplot(aes(Value, SalePrice)) +
     geom_point(size = .1) + geom_smooth(method = "lm", se=F, colour = "#FA7800") +
...
```{r}
boston_sub_200k <- boston_sub_200k %>% 
  mutate(Mansion = ifelse(R_BDRMS > 5, "Mansion", "Not_Mansion"))

reg2 <- lm(SalePrice ~ ., data = boston_sub_200k %>% 
                                 dplyr::select(SalePrice, LivingArea, 
                                                NUM_FLOORS,
                                               R_FULL_BTH, 
                                               R_KITCH, R_AC, R_FPLACE, Mansion))
