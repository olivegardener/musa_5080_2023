...
D <- 
  ...
  scale_fill_manual(values = palette5,
                    labels = qBr(totalPop09, "estimate"),
                    name = "Popluation\n(Quintile Breaks)") +
  labs(title = "Total Population", subtitle = "San Francisco Bay Area; 2009") +
  mapTheme() + 
  theme(plot.title = element_text(size=22))
...
```{r}
ggplot() + 
  geom_sf(data=tracts09, aes(fill = MedRent)) +
  # geom_sf(data = tracts09, aes(fill = )) +
...
BART_Stops_Buffers <- 
  rbind(
    st_buffer(BART_Stops_SF, (2 * 2640)) %>%
      mutate(Legend = "Buffer") %>%
      dplyr::select(Legend),
    st_union(st_buffer(BART_Stops_SF, (2 * 2640))) %>%
      st_sf() %>%
