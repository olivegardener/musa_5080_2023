...
Buffer4Crop <- st_union(st_buffer(BART_Stops_SF, (15*2640))) %>%
      st_sf() %>%
      mutate(Legend = "Crop Buffer")

# Get the bounding box of Buffer4Crop
bbox4crop <- st_bbox(Buffer4Crop)

# Crop allTracts using the bounding box of Buffer4Crop
cropped_allTracts <- st_crop(allTracts, bbox4crop)

# plot(cropped_allTracts, border = NA)
```
...
```{r}
#Calc centroids
tracts_centroids <- st_centroid(cropped_allTracts)
...
for (i in 1:nrow(tracts_centroids)) {
  ...
  nearest_index <- st_nearest_feature(centroid, BART_Stops_SF)
  nearest_point <- BART_Stops_SF[nearest_index, ]
  distance <- st_distance(centroid, nearest_point)
  
  # Store the distance
  nearest_distances[i] <- as.numeric(distance)
}

# Add the distances to the DATABIZNESS dataframe
cropped_allTracts$Dist2Bart <- nearest_distances
```

```{r}
ggplot() +
  geom_sf(data = cropped_allTracts, 
          aes(fill = Dist2Bart),  # Fill color based on Dist2Bart
          color = NA,
          alpha = 0.75) +
  # geom_sf(data = cropped_allTracts, 
  #         aes(),  
  ...
  scale_fill_distiller(palette = "Spectral") +  # Gradient fill from blue to red
  theme_minimal()
...
```{r Plot regional map}
ggplot()+
  geom_sf(data = cropped_allTracts, fill = "white", color = "black", size = 0.01) +
  geom_sf(data = st_centroid(propmap),
