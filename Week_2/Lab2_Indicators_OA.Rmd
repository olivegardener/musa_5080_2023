...
Filter out Farralon Islands for 2009
```{r Filter Farralon 1}
# Calculate the centroids
centroids <- st_centroid(tracts09)

# Extract the centroid coordinates
coords <- st_coordinates(centroids)

# Add the coordinates as new columns
tracts09$lon <- coords[, "X"]
tracts09$lat <- coords[, "Y"]

min_lon <- min(tracts09$lon)
...
#Crop instead? Coords chosen using epsg.io
#tracts09_crop <- st_crop(tracts09, xmin = 6034509.933356, xmax = 5957121.869415, ymin = 2080091.306697, ymax = 2137109.999123)
...
B <- 
  ggplot() +
  geom_sf(data = crop, aes(fill = q5(estimate))) +
...
```{r}
ggplot() + 
  geom_sf(data=st_union(BART_Stops_SF, size= 0.5)) +
...
# # Create a subset where serviceplanningstoptype is "SP"
# LightRailStops <- subset(BART_Stops_SF, serviceplanningstoptype == "SI")

```

```{r}
# ggplot() + 
#   geom_sf(data=LightRailStops, 
#           aes(colour = serviceplanningstoptype), 
#           show.legend = "point", size= 0.5) +
...
```{r}
ggplot() +
  geom_sf(data=BART_Stops_Buffers) +
...
tracts from `tracts09` that intersect the buffer of transit stop in
...
-   The intersection returns only the areas where both `buffer` and
    `tracts09` polygons overlap. This is often referred to as "clipping"

-   The intersection polygons are returns the full extend of any polygon
    in `tracts09` that touch (or intersect) the `buffer`. There is no
...
-   The intersection of polygons to centroids returns the full extent of
    any polygon in `tracts09` that has its center (called a centroid)
    within the `buffer`
...
1.  `selection1` uses `tracts09[buffer,]` uses the least code, but is
    often confusing unless you know the {sf} package well. In this
    usage, the intersecting rows of `tracts09` are selected for by
...
2.  `selection2` is a longhand way to do the same. This uses
    `tracts09[st_intersects(tracts09, buffer) %>% lengths > 0, ]` to
    spell out all of the steps used in the first method. This can be
    read as: Select all rows of the rows from `tracts09` where
...
3.  `selection3` takes a different approach and uses `st_join()` with
    ...
    `tracts09` where the polygons did not join. These are the polygons
...
polygon, to select whether a polygon from `tracts09` intersects the
...
3.  join the original `tracts09` data frame using the `GEOID` as the
