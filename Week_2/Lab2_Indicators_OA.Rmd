...
Filter out Farralon Islands fo 2009
```{r Filter Farralon 1}
...
Filter out Farralon Islands fo 2017
```{r Filter Farralon 2}
# Calculate the centroids
...
```{r}
ggplot() + 
  geom_sf(data=st_union(MUNI_Stops_sf, size= 0.5)) +
  geom_sf(data=MUNI_Stops_sf, 
...
LightRailStops <- subset(MUNI_Stops_sf, serviceplanningstoptype == "SI")
...
LightRailBuffers <- 
  rbind(
    st_buffer(LightRailStops, 2640) %>%
      mutate(Legend = "Buffer") %>%
      dplyr::select(Legend),
    st_union(st_buffer(LightRailStops, 2640)) %>%
      st_sf() %>%
...
```{r}
ggplot() +
  geom_sf(data=LightRailBuffers) +
  geom_sf(data=LightRailStops, show.legend = "point") +
  facet_wrap(~Legend) + 
...
```{r}
buffer <- filter(LightRailBuffers, Legend=="Unioned Buffer")
```
