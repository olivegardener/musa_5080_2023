...
## Wrangling 2009 Census Tract data for both San Francisco and the Bay Area

```{r tracts09, message = FALSE, warning = FALSE, results = "hide"}
tracts09_bay <- 
  ...
  mutate(pctWhite = ifelse(TotalPop > 0, Whites / TotalPop,0),
         ...
         year = "2009", density = drop_units((TotalPop / (area * 0.00000003587 )))) %>%
...
```
## Wrangling 2017 Census Tract data for both San Francisco and the Bay Area

```{r tracts17, message = FALSE, warning = FALSE, results = "hide"}
tracts17_bay <- 
  get_acs(geography = "tract", 
          variables = c("B25026_001E","B02001_002E",
                        "B15001_050E","B15001_009E",
  ...
  mutate(pctWhite = ifelse(TotalPop > 0, Whites / TotalPop,0),
         pctBachelors = ifelse(TotalPop > 0, ((FemaleBachelors + MaleBachelors) / TotalPop),0),
         pctPoverty = ifelse(TotalPop > 0, TotalPoverty / TotalPop, 0),
         year = "2017", density = drop_units((TotalPop / (area * 0.00000003587 )))) %>%
  dplyr::select(-Whites, -FemaleBachelors, -MaleBachelors, -TotalPoverty) 
...
```
### Connecting both years' tract data for both san Francisco and the Bay Area

```{r bind tracts, message = FALSE, warning = FALSE}
...
```{r Wrangle Transit Data, message = FALSE, warning = FALSE}
...
Buffer4Crop <- st_union(st_buffer(BART_Stops_bay, (8*2640))) %>%
      st_sf() %>%
...
ggplot(allTracts.group_sf)+
    ...
    labs(title = "Time/Space Groups", subtitle = "TOD vs Non-TOD") +
    facet_wrap(~year)+
    mapTheme() + 
...
ggplot(allTracts.group_sf)+
    ...
    labs(title = "Time/Space Groups", subtitle = "Total Population") +
    facet_wrap(~year)+
    mapTheme() + 
    theme(plot.title = element_text(size=22))
```
...
ggplot(allTracts.group_sf)+
    ...
    geom_sf(data = BART_Stops_sf, col = "yellow")+
    geom_sf(data = buffer_sf, lwd = 1, col = "red",  fill = "NA")+
    labs(title = "Time/Space Groups", subtitle = "Median Household Income") +
    facet_wrap(~year)+
    mapTheme() + 
    theme(plot.title = element_text(size=22))
```
...
ggplot(allTracts.group_sf)+
    ...
    geom_sf(data = BART_Stops_sf, col = "yellow")+
    geom_sf(data = buffer_sf, lwd = 1, col = "red",  fill = "NA")+
    labs(title = "Time/Space Groups", subtitle = "Median Rent") +
    facet_wrap(~year)+
    mapTheme() + 
    theme(plot.title = element_text(size=22))
```
...
ggplot(allTracts.group_sf)+
    ...
    geom_sf(data = BART_Stops_sf, col = "yellow")+
    geom_sf(data = buffer_sf, lwd = 1, col = "red",  fill = "NA")+
    labs(title = "Time/Space Groups", subtitle = "Density") +
    facet_wrap(~year)+
    mapTheme() + 
    theme(plot.title = element_text(size=22))
...
# kable(allTracts.Summary_bay) %>%
#   kable_styling() %>%
#   footnote(general_title = "\n",
#            general = "Table 2.2")
```

```{r TOD Indicator Table sf, message = FALSE, warning = FALSE}
allTracts.Summary_sf <- 
  st_drop_geometry(allTracts.group_sf) %>%
  group_by(year, TOD) %>%
  summarize(Percent_Poverty = mean(pctPoverty, na.rm = T),
            Population = mean(TotalPop, na.rm = T),
            #Percent_White = mean(pctWhite, na.rm = T),
            Density = mean(density, na.rm = T),
            Rent = mean(MedRent, na.rm = T))

# kable(allTracts.Summary_sf) %>%
#   kable_styling() %>%
#   footnote(general_title = "\n",
#            general = "Table 2.2")
```
...
bay_plot <- allTracts.Summary_bay %>%
  ...
  labs(title = "Bay Area Indicator differences across time and space") +
...
sf_plot <- allTracts.Summary_sf %>%
  ...
  facet_wrap(~Variable, scales = "free", ncol=5) +
  scale_fill_manual(values = c("#bae4bc", "#0868ac")) +
  labs(title = "San Francisco Indicator differences across time and space") +
  plotTheme() + theme(legend.position="bottom")
...
stops_pop_bay <- ggplot()+
  geom_sf(data = cropped_allTracts_bay, fill = "white") +
...
grid.arrange(stops_pop_bay, stops_rent_bay)
...
graduated_stops_sf <-
  st_join(Stops_Individual_sf, st_centroid(cropped_allTracts_sf)) %>%
  group_by(Name) %>%
  summarize(pop_0.5mi_sum = sum(TotalPop, na.rm = TRUE),
            rent_0.5mi_mean = round(mean(MedRent, na.rm = TRUE), digits = 0))

stops_pop_sf <- ggplot()+
  geom_sf(data = cropped_allTracts_sf, fill = "white") +
  geom_sf(data = BART_Stops_sf)+
  geom_sf(data = BART_Stops_sf_Buffers)+
...
grid.arrange(stops_pop_sf, stops_rent_sf)
```  
...
bay_lines <- ggplot(allTracts.rings.summary_bay,
      ...
      geom_point(size=3) + 
  geom_line(size=2)+
  labs(title = "Median Rent over distance to BART Lines, Bay Area") +
...
sf_lines <- ggplot(allTracts.rings.summary_sf,
      ...
      geom_point(size=3) + 
  geom_line(size=2)+
  labs(title = "Median Rent over distance to BART Lines, San Francisco") +
  scale_x_continuous(limits = c(0, 10))+
  scale_y_continuous(limits = c(0, 2100))
