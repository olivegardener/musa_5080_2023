...
```{r do_mutates, cache = TRUE}
allACS <- allACS %>%
  mutate(change_population = total_pop.2020 - total_pop.2016, 
...
We can then create a vector variable, which we call `mtAiry`, using a "boolean" statement.
...
ggplot(allACS)+
  ...
  geom_abline(intercept = 0, slope = 1)+
  labs(
    title = "2020 vacant housing units per tract a Function of 2016 vacant housing units per tract",
    caption = "Data: US Census Bureau, ACS 5-year estimates",
...
This plot shows vacant housing units per tract in 2016 vs 2020. From this plot, we can see generally...


2. A ggplot map symbolizing the number of vacant housing units in 2016 as a percentage of total housing units (e.g.  "total_HU.2016") on a per-tract basis. Outline the Mt. Airy neighborhood on the map. Annotate this map with a single sentence describing what it shows or implies about Mt Airy and Philadelphia.
...
4. Styling: Manipulate the code chunks so that you "fold" them in the final markdown (you might need to google how to do this). Title the markdown "Tidycensus Markdown", put your name and date in the header. Remove any extraneous tables and text from the original markdown that aren't needed. Adjust the table of contents as needed to include your new sections.
